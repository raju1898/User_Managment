...
public class CustomUserDetailsService implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		return jdbcTemplate.queryForObject(QUERY, new Object[] { username },
				(rs, rowNum) -> org.springframework.security.core.userdetails.User
						.withUsername(rs.getString("username")).password(rs.getString("password")) // should already be
																									// encrypted
	...
	public User addUser(User user) {
		...
		try {
			...
			jdbcTemplate.update(insertQuery, user.getUsername(), hashPassword, user.getRoles());
